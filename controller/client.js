  ...
  async function updateClientController(req, res){
    try{
      ...
      if(decoded.typeUser == 2){
        const update = await updateClientModel(data)
        if(update.affectedRows ==0){
          return res.status(404).json({ error: 'Dados do Client nao atualizados'});
        }
        return res.status(200).json(update);
      }else{
        return res.status(400).json({ error: 'Acesso negado'});
      ...
      console.log(err);
      return res.status(400).json({ error: 'Error ao atualizar dados do Client'});
    }
  }
